...
(defn handle-api-search [{:keys [card-server] :as request}]
  (let [{{query :q} :params} request
        server-snapshot @card-server]
    (-> (wiki.bc.card-server/resolve-text-search server-snapshot nil {:query_string query} nil)
        (json/write-str)
        (util/->json-response))))
...
(defn handle-api-search-autocomplete [{:keys [card-server] :as request}]
  (let [{{query :q} :params} request
        server-snapshot @card-server]
    (-> (wiki.bc.card-server/resolve-autocomplete-search server-snapshot nil {:query_string query} nil)
        (json/write-str)
        (util/->json-response))))
