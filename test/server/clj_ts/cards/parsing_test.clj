...
(deftest partition-raw-card-text_simple-card-text
  (let [raw-card-text "simple text"
        {:keys [source-type source-body]} (parsing/partition-raw-card-text raw-card-text)]
    (is (= source-type nil))
    (is (= source-body raw-card-text))))

(deftest raw-card-text->card-map_simple-card-text
  (let [raw-card-text "simple text"
        {:keys [source_type source_type_implicit? source_data]} (parsing/raw-card-text->card-map raw-card-text)]
